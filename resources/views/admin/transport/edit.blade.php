...
<html>
	...
	<body>
		...
		<div class="main-container">
            <h3 style="text-align: center; padding-top: 3rem;padding-bottom: 3rem;">Update Transport</h3>

            <div class="card-header">
                <h3 class="card-title">Update Transport</h3>
            </div>
            <form action="{{ route('home.trantsport.transport.update', $transport->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="card-body">
                    <div class="form-group">
                        <label for="exampleInputEmail1">Transport Name</label>
                        <input type="text" name="title" class="form-control" id="exampleInputEmail1" value="{{ $transport->title }}">
                    </div>
                    @error('title')
                    ...
                    @enderror
                    <div class="w-50">
                        <img src="{{ url('storage/' . $transport->image) }}" class="w-50 mb-3">
                    </div>
                    <div class="form-group">
