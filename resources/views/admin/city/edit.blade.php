...
<html>
	...
	<body>
		...
		<div class="main-container">
            <h3 style="text-align: center; padding-top: 3rem;padding-bottom: 3rem;">Create City</h3>

            <div class="card-header">
                <h3 class="card-title">Update City</h3>
            </div>
            <form action="{{ route('home.city.city.update', $city->id) }}" method="POST">
                @csrf
                @method('patch')
                <div class="card-body">
                    <div class="form-group">
                        <label for="exampleInputEmail1">City Name</label>
                        <input type="text" name="title" class="form-control" id="exampleInputEmail1" placeholder="Enter transport name" value="{{ $city->title }}">
                    </div>
                    @error('title')
                    ...
                    @enderror
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1" class="form-label">Your description about city</label>
                        <textarea class="form-control" name="desctiption" id="exampleFormControlTextarea1" rows="3">{{ $city->desctiption }}</textarea>
                    </div>
                    @error('desctiption')
                        <div class="d-block text-danger">
                            {{ $message }}
